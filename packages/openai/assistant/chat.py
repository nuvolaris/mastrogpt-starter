...
When you get a context and a query, try to reply with a small response.
The user will see the context in another window, so there is no need
for you to repeat what the context says.
Try to be polite and reply in italian. 
"""
...
def ask_openai(args, *, query=None, context=None):
    global AI
